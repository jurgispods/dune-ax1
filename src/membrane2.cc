...
#include<dune/ax1/membrane2/membrane2_bctype.hh>
#include<dune/ax1/membrane2/membrane2_bcextension.hh>
#include<dune/ax1/membrane2/membrane2_Pk.hh>
...
{
	std::cout<< "This is Membrane2. Geil." << std::endl;
  try
  {
    ...
    {
    if(helper.rank()==0)
      std::cout << "usage: ./membrane2 <level>" << std::endl;
    return 1;
    }
    ...
    std::valarray<double> dof(max_level), err(max_level);
    ...
    for ( int level=0; level<max_level; level++ )
    {
      ...
      if (helper.size()==1)
      {
        ...
        */
        ...
        Membrane<double> memb( 2.0 ); // just to let membrane2 still compile
        
        Physics<double> physics( elec1, memb, elec3 );
        
        // analytical solutions ##############################
        
        double dl = elec1.getDebyeLength();
        
        double boundl = physics.potential( -5.0, dl, -1 );
        double boundr = physics.potential(  5.0, dl,  2 );
        ...
        membrane2_Pk(gv, physics, position, solution);
        
        int half = ( solution.size() + 1 ) / 2;
        ...
        for ( int i=0; i<position.size(); i++ )
        {
          if ( position[i] <= 0.0 )
          {
            error[i] = solution[i] - physics.potential( position[i], dl, -1 );
            concentrationl[i] = physics.elecl.getConcentration (0, solution[i]);
            positionl[i] = position[i];
          }
          if ( position[i] >= 0.0 )
          {
            error[i] = solution[i] - physics.potential( position[i], dl,  2 );
            concentrationr[i-half+1] = physics.elecr.getConcentration (0, solution[i]);
            positionr[i-half+1] = position[i];
          }
        }
        
        err[level] = (abs(error)).max();
        
        // graphical output ##################################
        
        Output::gnuplotAppend("error.dat", (double) dof[level], err[level] );
    ...
    std::valarray<double> mdof(max_level-1), ord(max_level-1);
