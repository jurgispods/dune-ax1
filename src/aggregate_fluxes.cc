  ...
  while(std::getline(*file_in[masterFile], line))
  {
    ...
    // All fluxes have been aggregated, write to file
    {
      ...
      for(int i=0; i<nMembraneElements; i++)
      {
        ...
        double sum_na = single_values[Na][i] + single_values[Mori_Na][i];
        double sum_k = single_values[K][i] + single_values[Mori_K][i];
        double sum_cl = single_values[Cl][i] + single_values[Mori_Cl][i];
...
int main(int argc, char** argv)
{
  try{
    ...
    if (argc<2)
      ...
      if(helper.rank()==0)
      {
        debug_info << "usage: ./aggregateFluxes <flux_choice> [config-file]" << std::endl;
    ...
    if(configName == "default")
    {
      ...
      aggregate(params,config,flux_choice,inputDir,outputDir);
      configFound = true;
    }
    if(configName == "ES")
    {
