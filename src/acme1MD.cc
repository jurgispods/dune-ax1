...
void run(Grid& grid, Acme1MDParameters& params, Config& config, GV& gv, std::vector<double>& coords, double dtstart, double tend,
    ElementSubdomainMapper& elemSubdomainMapper, SubGV& elecGV, SubGV& membGV)
{
  ...
  //############# operator-split ######################
  Acme1MDSetup<Grid,GV,PHYSICS,SubGV> acme1MDSetup(grid, gv, physics, elecGV, membGV);
  acme1MDSetup.setup(dtstart, tend);
  //###################################################
}
  ...
  try{
    ...
    if (1 && helper.size()==1)
    {
      ...
      } else
      {
        ...
        if(params.doLocalRefine())
        {
          const int minLevelNearMembrane = 6;
          if(level < minLevelNearMembrane)
          {
            const int diffLevel = minLevelNearMembrane - level;
            double upperBound = coords[0] + 10.0;
            // Locally refine near membrane (i.e., until distance 10 from the membrane)
            Tools::localRefineVector( coords, diffLevel, upperBound);
          }
        }
      }
      ...
      if(configName == "default")
      {
        ...
        run(grid,params,config,gv,coords,dtstart,tend,
