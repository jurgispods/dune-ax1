...
{
	std::cout<< "This is Membrane4. Dick." << std::endl;
  try
  {
    ...
    for ( int level=minLevel; level<maxLevel; level++ )
    {
      ...
      if (helper.size()==1)
      {
        ...
        double d = physics.getMaxThicknessForDPhi( 1.5*dl, 2, 2.0/3.0*dPhi );
        ...
        */
        std::vector<double> coords2 = { 0.5*d, 5.0 };
        ...
        double boundl = physics.potential2( -5.0, dl, -1, -d, -4.0/3.0*dPhi );
        double boundr = physics.potential2(  5.0, dl,  2,  d,  2.0/3.0*dPhi );
        ...
        // L2 norm
        //err[level-minLevel] = Tools::normL2( position, error );
