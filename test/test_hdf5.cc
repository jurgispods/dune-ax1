...
int main(int argc, char** argv)
{
  try{
    ...
    if (1 && helper.size()==1)
    {
      ...
      std::vector<int> dims;
      dims.push_back(coords.size());
      dims.push_back(1);

      HDF5Tools::initialize(std::string("test_hdf5.h5"));
      HDF5Tools::writeVector(std::string("test_hdf5.h5"),
          dims,coords,std::string("coords"));

      HDF5Tools::writeVector(std::string("test_hdf5.h5"),
                dims,coords,std::string("coords_copy"));

      std::vector<double> bla(10000*1000);
