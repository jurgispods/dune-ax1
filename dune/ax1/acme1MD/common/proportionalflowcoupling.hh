...
#ifndef DUNE_AX1_PROPORTIONALFLOWCOUPLING_HH
#define DUNE_AX1_PROPORTIONALFLOWCOUPLING_HH
...
public:
  ...
  ( const IG& ig,
    ...
    R& r1, R& r2) const
  {
    debug_jochen << "ContinuousValueContinuousFlowCoupling @" << ig.geometry().center() << std::endl;

    // Extract leaf local function spaces from type tree
    typedef typename LFSU1::template Child<1>::Type LFSU1_POT;
    typedef typename LFSV1::template Child<1>::Type LFSV1_POT;

    const LFSU1_POT& lfsu1Pot = lfsu1.template child<1>();
    const LFSV1_POT& lfsv1Pot = lfsv1.template child<1>();

    // domain and range field type
    typedef typename LFSU1_POT::Traits::FiniteElementType::
      Traits::LocalBasisType::Traits::DomainFieldType DF;
    typedef typename LFSU1_POT::Traits::FiniteElementType::
      Traits::LocalBasisType::Traits::RangeFieldType RF;
    typedef typename LFSU1_POT::Traits::FiniteElementType::
    ...
    typedef typename LFSU1_POT::Traits::SizeType size_type;
    ...
    for (typename Dune::QuadratureRule<DF,dim-1>::const_iterator it=rule.begin(); it!=rule.end(); ++it)
      {
        ...
        // evaluate ansatz shape functions (assume Galerkin for now)
        std::vector<RangeType> phi1(lfsv1Pot.size());
        lfsv1Pot.finiteElement().localBasis().evaluateFunction(local1,phi1);

        std::vector<RangeType> phi2(lfsv2.size());
        lfsv2.finiteElement().localBasis().evaluateFunction(local2,phi2);

        // evaluate gradient of shape functions
        std::vector<JacobianType> js1(lfsu1Pot.size());
        lfsu1Pot.finiteElement().localBasis().evaluateJacobian(local1,js1);
        ...
        std::vector<Dune::FieldVector<RF,dim> > gradphi1(lfsu1Pot.size());
        for (size_t i=0; i<lfsu1Pot.size(); i++)
          {
            gradphi1[i] = 0.0;
        ...
        for (size_t i=0; i<lfsu1Pot.size(); i++)
          gradu1.axpy(x1(lfsu1Pot,i),gradphi1[i]);
        ...
        for (size_type i=0; i<lfsv2.size(); i++)
          {
            ...
            debug_jochen << "value2 = " << value << std::endl;
...
#endif /* DUNE_AX1_PROPORTIONALFLOWCOUPLING_HH */
