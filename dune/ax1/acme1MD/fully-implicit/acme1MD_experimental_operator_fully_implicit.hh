...
#ifndef DUNE_AX1_ACME1MD_EXPERIMENTAL_OPERATOR_FULLY_IMPLICIT_HH
#define DUNE_AX1_ACME1MD_EXPERIMENTAL_OPERATOR_FULLY_IMPLICIT_HH
...
class Acme1MDExperimentalOperatorFullyImplicit
      : public Dune::PDELab::NumericalJacobianVolume<Acme1MDExperimentalOperatorFullyImplicit
        <ParamCon,ParamPot,FiniteElementMapCon,FiniteElementMapPot> >,
        public Dune::PDELab::NumericalJacobianApplyVolume<Acme1MDExperimentalOperatorFullyImplicit
        <ParamCon,ParamPot,FiniteElementMapCon,FiniteElementMapPot> >,
        public Dune::PDELab::NumericalJacobianBoundary<Acme1MDExperimentalOperatorFullyImplicit
        <ParamCon,ParamPot,FiniteElementMapCon,FiniteElementMapPot> >,
        public Dune::PDELab::NumericalJacobianApplyBoundary<Acme1MDExperimentalOperatorFullyImplicit
        <ParamCon,ParamPot,FiniteElementMapCon,FiniteElementMapPot> >,
        ...
        public Dune::PDELab::InstationaryLocalOperatorDefaultMethods<typename ParamPot::Traits::RangeFieldType>
{
  ...
  public:
    ...
    Acme1MDExperimentalOperatorFullyImplicit (ParamCon& paramCon_, ParamPot& paramPot_, int intorderadd_=0)
      : paramCon(paramCon_),
        paramPot(paramPot_),
    ...
    {
      //debug_jochen << "PoissonNernstPlanckOperator::alpha_volume @ "<< eg.geometry().center() << std::endl;
      ...
      for(int j=0; j<NUMBER_OF_SPECIES; j++)
      {
        ...
        tensorSingleCon = paramCon.A(eg.entity(),localcenter, j);
      ...
      for (typename Dune::QuadratureRule<DF,dim>::const_iterator it=rule.begin(); it!=rule.end(); ++it)
      {
        ...
        for(int j=0; j<NUMBER_OF_SPECIES; j++)
        {
          ...
          typename ParamCon::Traits::RangeType bSingleCon = paramCon.b(eg.entity(),it->position(),graduPot,j);
          typename ParamCon::Traits::RangeFieldType cSingleCon = paramCon.c(eg.entity(),it->position(),j);
          typename ParamCon::Traits::RangeFieldType fSingleCon = paramCon.f(eg.entity(),it->position(),j);
                         ...
                         R& r_s) const
    {
      ...
      for(int j=0; j<NUMBER_OF_SPECIES; j++)
      {
        Dune::PDELab::ConvectionDiffusionBoundaryConditions::Type bctypeSingleCon
          = paramCon.bctype(ig.intersection(),facecenterlocal,j);
      ...
      for (typename Dune::QuadratureRule<DF,dim-1>::const_iterator it=rule.begin(); it!=rule.end(); ++it)
        {
          ...
          for(int j=0; j<NUMBER_OF_SPECIES; j++)
          {
            ...
            {
              // evaluate flux boundary condition
              typename ParamCon::Traits::RangeFieldType jSingleCon = paramCon.j(ig.intersection(),it->position(),j);
          ...
          for(int j=0; j<NUMBER_OF_SPECIES; j++)
          {
            if (bctypeCon[j]==Dune::PDELab::ConvectionDiffusionBoundaryConditions::Outflow)
              ...
              DUNE_THROW(Dune::Exception, "Outflow boundary not implemented!");
                ...
                // evaluate velocity field and outer unit normal
                typename ParamCon::Traits::RangeType bCon = paramCon.b(*(ig.inside()),local,graduPot,j);
                const Dune::FieldVector<DF,dim> n = ig.unitOuterNormal(it->position());

                // evaluate outflow boundary condition
                typename ParamCon::Traits::RangeFieldType oCon = paramCon.o(ig.intersection(),it->position(),j);
...
#endif /* DUNE_AX1_ACME1MD_EXPERIMENTAL_OPERATOR_FULLY_IMPLICIT_HH */
