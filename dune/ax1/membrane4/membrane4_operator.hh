...
class Membrane4LocalOperator :
public Dune::PDELab::NumericalJacobianApplyVolume<Membrane4LocalOperator<BCType> >,
public Dune::PDELab::NumericalJacobianVolume<Membrane4LocalOperator<BCType> >,
public Dune::PDELab::NumericalJacobianApplyBoundary<Membrane4LocalOperator<BCType> >,
public Dune::PDELab::NumericalJacobianBoundary<Membrane4LocalOperator<BCType> >,
...
public:
	...
	Membrane4LocalOperator (const BCType& bctype_, // needs boundary cond.type
                          Physics<double>& physics_,
                          unsigned int intorder_=2)
    : bctype(bctype_), physics(physics_), intorder(intorder_) {}
	...
	void alpha_volume (const EG& eg, const LFSU& lfsu, const X& x, const LFSV& lfsv, R& r) const
	{
      ...
      if ( globalpos <= physics.getMembranePosL() ) // left electrolyte
      {
        f = physics.elecl.rhsPoissonBoltzmann(u);
        epsilon = physics.elecl.getPermittivity();
      }
      else if ( globalpos >= physics.getMembranePosR() ) // right electrolyte
      {
        f = physics.elecr.rhsPoissonBoltzmann(u);
        epsilon = physics.elecr.getPermittivity();
      }
      else // membrane
      {
        f = 0.0;
        epsilon = physics.membrane.getPermittivity();
      }
      //debug_verb << "f= " << f << std::endl;
