...
#ifndef DUNE_AX1_ACME2CYL_DEFAULT_CONFIG_HH
#define DUNE_AX1_ACME2CYL_DEFAULT_CONFIG_HH
...
class DefaultConfiguration
{
  public:
    ...
    struct Traits
    {
      ...
      using NERNST_PLANCK_PARAMETERS = DefaultNernstPlanckParameters<GV,RF,PHYSICS,INITIAL_CON,GF_MEMB_FLUX,GF_MORI_FLUX>;

      template<typename PHYSICS>
      using POISSON_PARAMETERS = DefaultPoissonParameters<GV,RF,PHYSICS>;
    ...
    DefaultConfiguration(const Acme2CylParameters& params)
    : name("default"),
...
template<typename T>
const double DefaultConfiguration<T>::LENGTH_SCALE = 1.0e-9; // nano meters

template<typename T>
const double DefaultConfiguration<T>::TIME_SCALE   = 1.0e-6; // micro seconds

template<typename T>
const double DefaultConfiguration<T>::con_diffWaterNa = 1.33e-9; // m2 / s
template<typename T>
const double DefaultConfiguration<T>::con_diffWaterK  = 1.96e-9; // m2 / s
template<typename T>
const double DefaultConfiguration<T>::con_diffWaterCl = 2.03e-9; // m2 / s

template<typename T>
const double DefaultConfiguration<T>::reduction = 1e-5;
template<typename T>
const double DefaultConfiguration<T>::absLimit = 1e-5;
