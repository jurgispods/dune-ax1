...
//      DUNE_THROW(Dune::Exception, "KAKCEEEEEEEEEEEEEEEE");
  ...
  int getSubDomainElementIndex(const SubDomainElement& se) const
  {
    ...
    if(elecGV.contains(se))
      return elecElementMapper.index(se);
    else
      return membElementMapper.index(se);
  }
  ...
  void gridInfo() const
  {
    const SubGV& elecGV = gv.grid().subDomain(0).leafGridView();
    const SubGV& membGV = gv.grid().subDomain(1).leafGridView();
  ...
  T getPermittivity (const Element& e) const
  {
    if ( not isMembrane(e) )
    {
      ...
      int sdElemIndex = membElementMapper.index(*sep);
