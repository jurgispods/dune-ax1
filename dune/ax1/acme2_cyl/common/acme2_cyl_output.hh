...
class Acme2CylOutput
{
  public:
    ...
    struct GFS_ION
    {
      typedef Dune::PDELab::GridFunctionSubSpace<GFS_CON,Dune::TypeTree::TreePath<SPECIES> > type;
    };
      ...
      useMori(physics.getParams().useMori())
    {
      ...
      if(gv.comm().rank() == 0)
      {
        ...
        if(access(physics.getParams().getOutputPrefix().c_str(),0) != 0 
            && !physics.getParams().general.get("suppressAllOutput",false))
        {
          ...
          for(int i=0; i<outputDirs.size(); i++)
          {
            ...
            if(access((physics.getParams().getOutputPrefix() + outputDirs[i]).c_str(),0) != 0)
            {
              ...
              if(status != 0 && !physics.getParams().general.get("suppressAllOutput",false))
        ...
        if(physics.getParams().createGridFile() || physics.getParams().doEquilibration())
        {
          ...
          if(! out_basegrid.good() && !physics.getParams().general.get("suppressAllOutput",false))
