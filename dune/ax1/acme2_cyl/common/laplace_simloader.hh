...
#ifndef DUNE_AX1_LAPLACE_SIMULATIONLOADER_HH
#define DUNE_AX1_LAPLACE_SIMULATIONLOADER_HH
...
class LaplaceSimulationLoader
{
  public:
    ...
    LaplaceSimulationLoader(const PHYSICS& physics_,
        const GV& gv_,
        const SubGV& elecGV_,
    ...
    void transferData(std::vector<Ax1SimulationState<Real> >& states, const MultiGFS& multigfs_Equi)
    {
      ...
      GFS_POT_SUB gfsPotSub_Equi(multigfs_Equi);

      // Load solution vectors (one per loaded simulation state)
      typedef typename Dune::PDELab::BackendVectorSelector<MultiGFS,Real>::Type U_Equi;
      std::vector<U_Equi> uold_Equi(states.size(), U_Equi(multigfs_Equi,0.0));
      std::vector<U_Equi> unew_Equi(states.size(), U_Equi(multigfs_Equi,0.0));

      // One potential/concentration gridfunction for each loaded solution vector
      typedef Dune::PDELab::DiscreteGridFunction <GFS_POT_SUB,U_Equi> DGF_POT_Equi;
      std::vector<Dune::shared_ptr<DGF_POT_Equi> > dgfOldPot_Equi;
      std::vector<Dune::shared_ptr<DGF_POT_Equi> > dgfNewPot_Equi;
      ...
      for(int j=0; j<states.size(); j++)
      {
        ...
        std::size_t i = 0;
        for (typename U_Equi::iterator it = uold_Equi[j].begin(); it != uold_Equi[j].end(); ++it)
        {
           *it = stdVec[i];
           i++;
        }
        ...
        i = 0;
        for (typename U_Equi::iterator it = unew_Equi[j].begin(); it != unew_Equi[j].end(); ++it)
        {
           *it = stdVec[i];
           i++;
        ...
        Dune::shared_ptr<DGF_POT_Equi> dgfOldPot_Equi_ptr(new DGF_POT_Equi(gfsPotSub_Equi, uold_Equi[j]));
        dgfOldPot_Equi.push_back(dgfOldPot_Equi_ptr);

        Dune::shared_ptr<DGF_POT_Equi> dgfNewPot_Equi_ptr(new DGF_POT_Equi(gfsPotSub_Equi, unew_Equi[j]));
      ...
      if(x.size() == 2 && doYCoordinatesMatch)
      {
        ...
        Ax1CoarseToFineGridTransferGridFunction<GV,DGF_POT_Equi,PHYSICS>
        ...
        if(dgfOldPot_Equi.size() > 1)
          DUNE_THROW(Dune::Exception, "Multiple gridfunction interpolation not implemented when!");
...
//        Dune::PDELab::interpolate(dgfNewCon_FineGrid, gfsCon, solutionVectors.unew);
        ...
        // Define functions which interpolate the equilibration values onto the new (finer) elecGV
        Ax1CoarseGridInterpolationGridFunction<GV,DGF_POT_Equi,PHYSICS>
    ...
    void extrapolateData(std::vector<Ax1SimulationState<Real> >& states)
    {
      ...
      if(x.size() != 2 || !doYCoordinatesMatch)
        debug_warn << "Equilibration grid has more than 1 element in x-direction! Trying to load data with old "
          << "(slow) extrapolation strategy..." << std::endl;
      ...
      coords[0] = dynamic_cast<std::vector<double>&>(x);
      coords[1] = dynamic_cast<std::vector<double>&>(y);
      ...
      typedef Ax1YaspPartition<2> YP;
      Dune::array<int,2> yasppartitions;
      yasppartitions[0] = px;
      yasppartitions[1] = py;
      ...
      Dune::GridPtr<BaseGrid> yaspPtr;
      //yaspPtr = new BaseGrid(L,N,periodic,overlap,gv.comm(),yp);
      yaspPtr = new BaseGrid(coords,periodic,overlap,gv.comm(),yp);
      ...
      typedef Ax1GridGenerator::HostGrid HostGrid;
      //HostGrid equilibrationGrid(*yaspPtr,coordFunction);
      HostGrid& equilibrationGrid = *yaspPtr;
...
//        }
      ...
      Ax1FineToCoarseGridTransferGridFunction<GV,Ax1SubdomainIndexGridFunction<GV,Real,PHYSICS>,
          PHYSICS> subdomainIndexGF_Equi_oneXElem(equilibrationGV,subdomainIndexGF,physics,1,y.size()-1); // nElementsXCoarse = 1
      ...
      // Reconstruct pot GFS
      typename Acme2CylTraits::FEM femPot(equilibrationGV);
      typename Acme2CylTraits::GFS gfsPot_Equi_TEMP(equilibrationGV,femPot);

      // Reconstruct con GFS
      typedef typename Acme2CylTraits::FEM FEM_CON;
      typedef Dune::PDELab::GridFunctionSpace<SubGV,FEM_CON,typename Acme2CylTraits::CONSTRAINTS,
          typename Acme2CylTraits::VBE> GFS_SINGLE_CON;
      typedef Dune::PDELab::PowerGridFunctionSpace<
        GFS_SINGLE_CON, NUMBER_OF_SPECIES, typename Traits::VBE, Dune::PDELab::EntityBlockedOrderingTag> GFS_CON;

      FEM_CON femCon(equilibrationGV);
      GFS_SINGLE_CON gfsCon_Equi_Single_TEMP(elecGV_Equi,femCon);
      GFS_CON gfsCon_Equi_TEMP(gfsCon_Equi_Single_TEMP);

      // Reconstruct MultiGFS
#ifdef MULTIPLE_MEMBRANE_ELEMENTS
      typedef Dune::PDELab::ISTLVectorBackend<> VBE_MULTI;
      typedef Dune::PDELab::MultiDomain::MultiDomainGridFunctionSpace<typename Acme2CylTraits::GridType,VBE_MULTI,
          Dune::PDELab::PermutationOrderingTag,GFS_CON,GFS_POT> MultiGFS_Equi;
      ...
      MultiGFS_Equi multigfs_Equi(md_equilibrationGrid, permutation,
...
//        }
#else
      typedef Dune::PDELab::ISTLVectorBackend<Dune::PDELab::ISTLParameters::static_blocking,AX1_BLOCKSIZE> VBE_MULTI;
      // Use InterleavedOrdering
      typedef Dune::PDELab::MultiDomain::MultiDomainGridFunctionSpace<typename Acme2CylTraits::GridType,VBE_MULTI,
          Dune::PDELab::InterleavedOrderingTag,GFS_CON,typename Traits::GFS> MultiGFS_Equi;
      MultiGFS_Equi multigfs_Equi(md_equilibrationGrid, {NUMBER_OF_SPECIES,1}, gfsCon_Equi_TEMP, gfsPot_Equi_TEMP);
#endif

      // TODO Make it possible to also load from a Laplace simulation, not only PNP simulation
//      // Reconstruct pot GFS
//      typename Acme2CylTraits::FEM femPot;
//      typename Acme2CylTraits::GFS gfsPot_Equi_TEMP(equilibrationGV,femPot);
//
//      // LexicographicOrderingTag
//      typename Acme2CylTraits::MultiGFS multigfs_Equi(md_equilibrationGrid,gfsPot_Equi_TEMP);

      // Do actual data transfer
      transferData(states, multigfs_Equi);
