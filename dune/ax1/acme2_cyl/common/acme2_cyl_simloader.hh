...
//        Dune::PDELab::interpolate(dgfNewCon_FineGrid, gfsCon, solutionVectors.unew);
    ...
    void extrapolateData(std::vector<Ax1SimulationState<Real> >& states)
    {
      ...
      Dune::shared_ptr<BaseGrid> yaspPtr;
      if(px*py != gv.comm().size() && px*py > 1)
        debug_warn << "px*py (" << (px*py) << " != gv.comm().size() (" << gv.comm().size()
          << ") when trying to load YaspGrid. This is not supported. Trying to continue anyway... " << std::endl;

      if(px*py == 1)
      {
        yaspPtr = std::make_shared<BaseGrid>(coords,periodic,overlap,MPI_COMM_SELF,yp);
      } else {
        yaspPtr = std::make_shared<BaseGrid>(coords,periodic,overlap,gv.comm(),yp);
      }
...
#endif
      debug_jochen << "Loaded HostGrid." << std::endl;
