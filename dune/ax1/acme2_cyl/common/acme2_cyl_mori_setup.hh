...
//      Dune::PDELab::constraints(bctypeCon, gfsConCopy, ccCon); // assemble constraints
      ...
      typedef std::array<std::array<std::size_t,2>,2> EntriesPerRow;

      typedef Dune::PDELab::istl::BCRSMatrixBackend<EntriesPerRow> MBE;
      //MBE mbe(
      //  {{
      //    {9*NUMBER_OF_SPECIES, 0},
      //    {0, 9}
      //  }}
      //);

      EntriesPerRow entriesPerRow;
      entriesPerRow[0][0] = 9*NUMBER_OF_SPECIES;
      entriesPerRow[0][1] = 0;
      entriesPerRow[1][0] = 0;
      entriesPerRow[0][0] = 9;
      MBE mbe(entriesPerRow);
      ...
      GO0_CON go0Con(multigfs,multigfs,cc,cc,mbe,elecSubProblemCon);
      GO0_POT go0Pot(multigfs,multigfs,cc,cc,mbe,elecSubProblemPot);
      ...
      DUMMY_OPERATOR dummyOperator(multigfs,multigfs,cc,cc,mbe,elecSubProblemCon,elecSubProblemPot);
      ...
      typedef typename DUMMY_OPERATOR::Traits::Jacobian Matrix;
      Dune::shared_ptr<Matrix> m_ptr(new typename DUMMY_OPERATOR::Traits::Jacobian(dummyOperator));

      debug_info << " +++++++++++++++++++++++ Pattern statistics ++++++++++++++++++++++++++ " << std::endl;
      const int nMacroBlocks = 4;
      for(int i=0; i<nMacroBlocks; ++i)
      {
        debug_info << m_ptr->patternStatistics()[i] << std::endl;
      }
      debug_info << " +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ " << std::endl;
...
//      LS ls(multigfs,cc,perm,inv_perm,maxLinIt,1);
      ...
      typedef Dune::PDELab::ISTLBackend_OVLP_BCGS_ILUn<typename Traits::GFS_POT,CC_POT> LS_POT;
      typedef Dune::PDELab::ISTLBackend_OVLP_BCGS_ILUn<typename Traits::GFS_CON,CC_CON> LS_CON;
      LS_POT lsPot(gfsPotCopy,ccPot,1,maxLinIt,5);
      LS_CON lsCon(gfsConCopy,ccCon,1,maxLinIt,5);
