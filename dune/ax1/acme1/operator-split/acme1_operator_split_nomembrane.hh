...
#ifndef ACME1_OPERATOR_SPLIT_HH
#define ACME1_OPERATOR_SPLIT_HH
...
class Acme1OperatorSplit
{
  public:
    ...
    Acme1OperatorSplit(U_CON& uoldCon_, U_CON& unewCon_,
      ...
      : uoldCon(uoldCon_),
        ...
        gfsCon(gfsCon_),
        gfsPot(gfsPot_),
        dgfCon(physics_,gfsCon,unewCon),
        dgfConPrevious(physics_,gfsCon,uConPrevious),
    ...
    bool converged()
    {
      ...
      else // Use max change in any unknown over the entire domain
      {
        ...
        debug_verb << maxDiffPot << "  (" << maxDiffPot << ")  ";
        debug_verb << maxDiffCon << "  (" << maxDiffCon << ")";

        debug_verb << " || " << defectPot << "  (" << defectCon << ")"
            << std::endl;
