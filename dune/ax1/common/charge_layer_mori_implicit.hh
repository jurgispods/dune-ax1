          ...
          if(membraneActive[membNumber])
          {
            ...
            if(true /*mIndex == (membGV.size(0)/2 - 1)*/)
            {
              ...
              for(int j=jLow; j<=jUp; j++)
              {
                ...
                /* Important: My implementation of membrane fluxes represents the ion-centric view, i.e. the sign
                 * tells whether ions of this species are flowing in or out of the membrane (in global coordinates).
                 * Therefore, as in the ionic fluxes case, the capacitive current (density), (sigma_new-sigma_old)/real_dt,
                 * must be scaled by 1/(e*N_A*z_j); 1/(e*N_A) is already contained in 'surfaceChargeScale', but z_j must
                 * be included manually here!
                */
                const int z_j = physics.getValence(j);
                const T moriFlux = (surfaceChargeScale / z_j) * (sigma_new - sigma_old) / real_dt;
