...
class Ax1GridGenerator
{
  public:
    ...
    // Grid stuff
    enum { dim=2 };
    typedef Ax1YaspPartition<dim> YP;
    // Or: use existing load balancer from dune-grid
    //typedef Dune::YLoadBalanceBackup<dim> YP;
...
//#endif
    ...
    // Since YaspGrid now supports entities of all codims, 'Dune::mdgrid::CellAndVertexCodims' should not
    // be speciefied anymore for the multidomaingrid traits. It now gves a compile-time error!
        ...
        Ax1ElementGroupMapper& elemGroupMapper, ElementSubdomainMapper& elemSubdomainMapper)
    {
      ...
      //Dune::shared_ptr<BaseGrid> yaspGridPtr(new BaseGrid (L,N,periodic,overlap,helper.getCommunicator(),yp));
      Dune::shared_ptr<BaseGrid> yaspGridPtr(new BaseGrid (coords,periodic,overlap,helper.getCommunicator(),yp));
...
//      hostGrids.push_back(hostGridPtr);
      ...
      HostGrid& hostGrid = *yaspGridPtr;

      int i=0;
      debug_jochen << " ------------------------------------------- " << std::endl;
//      for (const auto& e : elements(hostGrid.leafGridView()))
//      {
//        debug_jochen << "Element #" << i << " @ " << e.geometry().center() << std::endl;
//        ++i;
//      }
      debug_jochen << " ------------------------------------------- " << std::endl;
...
#warning "Selected grid could not be found!"
  ...
  //      }
      ...
      double ymin = params.yMax();
      double ymax = params.yMin();
      ...
      for(ElementIterator ep = gv.begin<0>(); ep != gv.end<0>(); ++ep)
      {
        ...
        if(ep->partitionType() == Dune::PartitionType::InteriorEntity)
        {
          ...
          ymin = std::min(ep->geometry().corner(0)[1], ymin);
          ymax = std::max(ep->geometry().corner(ep->geometry().corners()-1)[1],ymax);

        }

        int elemIndex = elementMapper.index(*ep);
      ...
      debug_info << "Processor " << helper.rank() << " has x interval [" << xmin  << " " << xmax << "]"
        << " and y interval [" << ymin  << " " << ymax << "]" << std::endl;
      ...
      {
        int elemIndex = elementMapper.index(*eit);
