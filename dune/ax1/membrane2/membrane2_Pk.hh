...
#include <dune/ax1/membrane2/membrane2_operator.hh>

template<class GV>
void membrane2_Pk (const GV& gv, Physics<double>& physics,
                   std::valarray<double>& x, std::valarray<double>& f)
{
  ...
  std::cout << "constrained dofs=" << cc.size() 
	...
	typedef Membrane2LocalOperator<BCTypeParam> LOP;                // operator including boundary
  ...
  /*
	...
	vtkwriter.write("membrane2_Pk",Dune::VTKOptions::binaryappended);       // <= NEW
  */
  
	Tools::getSolutionVector(udgf, 0, x, f);
