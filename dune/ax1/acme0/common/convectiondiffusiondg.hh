...
/*
 * This file was copied from <dune/pdelab/localoperator/convectiondiffusiondg.hh>
 * so that it can be modified for testing purposes without affecting the original one
 */
#ifndef DUNE_AX1_ACME0_CONVECTIONDIFFUSIONDG_HH
#define DUNE_AX1_ACME0_CONVECTIONDIFFUSIONDG_HH
  ...
  namespace PDELab {
    ...
    public:
        ...
        for (typename Dune::QuadratureRule<DF,dim>::const_iterator it=rule.begin(); it!=rule.end(); ++it)
          {
            ...
            lfsu.finiteElement().localBasis().evaluateFunction(it->nodePositions(),phi);
#else
            ...
            lfsu.finiteElement().localBasis().evaluateJacobian(it->nodePositions(),js);
#else
                           ...
                           R& r_s, R& r_n) const
      {
        ...
        // exactly one of both elements is membrane element
//        if(membraneBoundary && param.getName() == "NernstPlanckParameters")
//        {
//          debug_verb << "--------------------[" << param.getName() << "]----------------------" << std::endl;
//          debug_verb << "Membrane boundary @ " << ig.geometry().center() << std::endl;
//          debug_verb << "weights = " << (weights==ConvectionDiffusionDGWeights::weightsOn) << std::endl;
//          debug_verb << "harmonic_average = " << harmonic_average << std::endl;
//          debug_verb << "penalty_factor = " << penalty_factor << std::endl;
//        }
        ...
        for (typename Dune::QuadratureRule<DF,dim-1>::const_iterator it=rule.begin(); it!=rule.end(); ++it)
          {
            ...
            typename T::Traits::RangeType b = 0.5*(param.b(*(ig.inside()),iplocal_s)+param.b(*(ig.outside()),iplocal_n));
...
//            if(//membraneBoundary &&
//              param.getName() == "NernstPlanckParameters")
//            {
//              debug_verb << "  Quadrature point [" << ig.geometry().global(it->position())
//                  << "]" << std::endl;
//              debug_verb << "  term1 = " << term1 << std::endl;
//              debug_verb << "  term2 = " << term2 << std::endl;
//              debug_verb << "  term3 = " << term3 << std::endl;
//              debug_verb << "  term4 = " << term4 << std::endl;
//            }
        ...
        for (typename Dune::QuadratureRule<DF,dim>::const_iterator it=rule.begin(); it!=rule.end(); ++it)
          {
            ...
            lfsv.finiteElement().localBasis().evaluateFunction(it->nodePositions(),phi);
#else
            const std::vector<RangeType>& phi = cache[order].evaluateFunction(it->position(),lfsv.finiteElement().localBasis());
#endif
...
#endif /* DUNE_AX1_ACME0_CONVECTIONDIFFUSIONDG_HH */
